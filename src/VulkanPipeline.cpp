...
void VulkanPipeline::createPipelineCache()
{
    ...
    result = vkCreatePipelineCache(deviceObj->device, &pipelineCacheInfo,
...
bool VulkanPipeline::createPipeline(VulkanDrawable *drawableObj, VkPipeline *pipeline, VulkanShader *shaderObj, VkBool32 includeDepth, VkBool32 includeVi)
{
    ...
    // Create the viweport state create info and provide the
    // number of viewport and scissors being used in the
    // rendering pipeline
    ...
    viewportStateInfo.viewportCount = NUMBER_OF_VIEWPORT;
    ...
    VkResult result;
    result = vkCreatePipelineLayout(deviceObj->device, &pPipelineLayoutCreateInfo,
    ...
    if (vkCreateGraphicsPipelines(deviceObj->device, pipelineCache, 1, &pipelineInfo,
